...
window.MurmurationModules.World = class World {
   ...
   *  Ported from attrition-private (19ce0b7) — same shared world.js, same
   *  bug: wall.thickness was a flat 12px that never rescaled, so a much
   *  smaller canvas made it a proportionally huge, harder-to-cross barrier.
   *  Zone radius (economy.js) is FIXED on purpose (a1bc27a, a permanent
   *  feature Ghost asked to keep) — the wall is the opposite case, a real
  ...
  drawWall(ctx) {
    ...
    // Ported from attrition-private (11b6c47) — see that commit for the full
    // story. Short version: the two outer glow bands already scaled off
    // `half`, but the brightest core stroke and the gate markers/font were
    // flat pixel constants tuned for thickness=12 (half=6), so onResize()
    // moving `half` didn't visibly move the wall's most eye-catching lines.
